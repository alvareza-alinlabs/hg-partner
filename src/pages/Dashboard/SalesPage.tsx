        ...
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            <div className="flex flex-col min-w-0">
              <h1 className="text-3xl font-black text-[#00172D] tracking-tight whitespace-nowrap truncate">Sales Internal</h1>
              <p className="text-gray-500 mt-1 font-medium text-xs uppercase tracking-widest whitespace-nowrap truncate">Daftar Tim Penjualan</p>
            </div>
          </div>
        </motion.div>
          ...
          <div className="flex gap-2 shrink-0 justify-end">
            ...
            <div className="relative z-30 shrink-0">
              ...
              <div className={`absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl flex flex-col overflow-hidden transition-all ${isActionMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                 ...
                 <button onClick={() => exportData(filteredClients, 'data_sales', 'csv')} className="px-4 py-3 text-left text-xs font-bold text-gray-700 hover:bg-gray-50 uppercase tracking-widest border-b border-gray-50 flex items-center gap-2">
                    <FileText className="w-3.5 h-3.5 text-gray-400" /> File CSV
                 </button>
                 <button onClick={() => exportData(filteredClients, 'data_sales', 'json')} className="px-4 py-3 text-left text-xs font-bold text-gray-700 hover:bg-gray-50 uppercase tracking-widest flex items-center gap-2">
              ...
              <AnimatePresence>
                {filteredClients.map((client) => (
                  ...
                  >
                    ...
                    <h3 className="text-base md:text-lg font-bold text-[#00172D] mb-0.5 md:mb-1">{client.nama_pic}</h3>
                    ...
                    <div className="mt-auto space-y-2 pt-3 md:pt-4 border-t border-gray-50 flex-1">
                      <div className="flex items-center gap-2 text-[11px] md:text-xs font-semibold text-gray-600">
                        <Building className="w-3 h-3 md:w-3.5 md:h-3.5 text-[#35ACDF]" />
                        <span className="truncate">{client.perusahaan}</span>
                      </div>
                      <div className="flex items-center gap-2 text-[11px] md:text-xs font-semibold text-gray-600">
                        <Phone className="w-3 h-3 md:w-3.5 md:h-3.5 text-[#35ACDF]" />
                        <span>{client.no_hp || client.telepon_kantor}</span>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
                       ...
                       <div className="min-w-0">
                         <h3 className="font-bold text-[#00172D] text-base md:text-lg truncate">{client.nama_pic}</h3>
                         <p className="text-[11px] md:text-xs font-semibold text-gray-500 truncate">{client.perusahaan} &bull; {client.provinsi}</p>
                       </div>
                    </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left whitespace-nowrap min-w-[600px]">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500">Nama Sales</th>
                      <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500">Perusahaan / Kontak</th>
                      <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500">Area</th>
                    </tr>
                  </thead>
                    ...
                    {filteredClients.map((client) => (
                      <tr key={client.id} onClick={() => navigate(`/dashboard/client/${client.id}`)} className="hover:bg-gray-50/50 cursor-pointer transition-colors group">
                        ...
                        <td className="px-4 md:px-6 py-3 md:py-4">
                          ...
                          <div className="text-[9px] md:text-[10px] font-semibold text-gray-400 mt-0.5">{client.no_hp || client.telepon_kantor}</div>
                        </td>
                        <td className="px-4 md:px-6 py-3 md:py-4">
                           <div className="text-[9px] md:text-[10px] font-bold text-gray-500 uppercase tracking-widest">{client.kota}</div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
